...
/**
 * Jenkins Pipeline {@link GlobalVariable} that exposes the {@code VectorCASTAdditionalTools} global in Pipeline scripts.
 *
 ...
 * <pre>{@code
 * def (found, fail, unstable) = VectorCASTAdditionalTools.checkBuildLogForErrors("build.log")
 * }</pre>
 *
 * <p>The behavior of {@code VectorCASTAdditionalTools} is implemented by {@link VectorCASTAdditionalToolsBridge}, which loads
 * a Groovy implementation from the workspace (if present) or falls back to a plugin-bundled default.</p>
 */
@Extension
public class VectorCASTAdditionalToolsGlobal extends GlobalVariable {
    ...
    public String getName() {
        return "VectorCASTAdditionalTools";
    }

    /**
     * Returns the per-build value bound to the {@code VectorCASTAdditionalTools} global.
     *
    ...
    public Object getValue(CpsScript script) {
        return new VectorCASTAdditionalToolsBridge(script);
    }
}
