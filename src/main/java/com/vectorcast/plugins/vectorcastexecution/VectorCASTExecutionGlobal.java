 ...
 * {@code VectorCASTExecution} global in Pipeline scripts.
 *
 ...
 *      = VectorCASTExecution.setSetupCommand(VC)
 * }</pre>
 *
 * <p>The behavior of {@code VectorCASTExecution} is implemented by
 * {@link VectorCASTExecutionBridge}, which loads
 * a Groovy implementation from the workspace (if present) or
 * falls back to a plugin-bundled default.</p>
 */
@Extension
public class VectorCASTExecutionGlobal extends GlobalVariable {
    ...
    public String getName() {
        return "VectorCASTExecution";
    }

    /**
     * Returns the build value bound to the {@code VectorCASTExecution} global.
     *
    ...
    public Object getValue(@NonNull final CpsScript script) {
        return new VectorCASTExecutionBridge(script);
    }
}
