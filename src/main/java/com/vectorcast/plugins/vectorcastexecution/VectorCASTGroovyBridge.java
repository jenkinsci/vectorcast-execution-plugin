 ...
 * Groovy implementation bundled with the plugin.
 *
 * <p>This bridge avoids calling Pipeline steps
 * (e.g. {@code load}, {@code writeFile}) from Java because Java
 * code is not CPS-transformed.
 * Instead, it compiles a Groovy source resource and
 * instantiates a known class.</p>
 *
 * <h2>Contract</h2>
 * <ul>
 *   <li>The bundled Groovy resource must be
 *   readable from {@code resourcePath}.</li>
 *   <li>The bundled Groovy must define {@code implClassName}.</li>
 *   <li>The impl class must have a 1-arg constructor
 *   accepting the Pipeline {@link CpsScript} (as {@code Object}).</li>
 * </ul>
 *
 * <p>Subclasses typically only define constants for the Groovy
 * resource path and implementation class name,
...
public abstract class VectorCASTGroovyBridge implements Serializable {
    @Serial
    private static final long serialVersionUID = -3676921768666753075L;

    /** Pipeline script context (CPS). Passed to the Groovy implementation. */
    private final CpsScript script;

    /** Classpath resource containing the Groovy implementation.
     * Must start with '/'.
     */
     ...
     * (recreated after Pipeline resume).
     */
     ...
     *
     * @param inputScript Pipeline {@link CpsScript} context (non-null)
     * @param inputResourcePath classpath resource
     *                     path to Groovy source (non-null, begins with '/')
